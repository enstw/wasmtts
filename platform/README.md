...
- `asr-listening-gate.py`、`asr-baseline/`：以固定 revision 的 multilingual Whisper 聽回 Matcha WAV，計算正規化 CER，並同時套用絕對上限與相對正式 baseline 的退化上限。這是可懂度回歸 gate，不等同主觀自然度盲聽。`current.json` 對應研究序列(noise 1／silence 0.2),`product.json` 對應產品配方腿(讀 `matcha-assets.json` `synthesis` 區塊,2026-08-16 凍結,CER 2.04%);兩條腿各自比對自己的 baseline,數字不可混報。
...
- `generate-matcha-lexicon-traditional.mjs`、`matcha-lexicon-traditional-curation.json`、`matcha-lexicon-traditional.txt`（＋`.meta.json`）:繁體 lexicon 鏡像。上游 lexicon 的多字詞條只有簡體(会计 kuai4 ji4、银行 yin2 hang2),繁體直輸落到單字 fallback 才產生 會計 hui4/銀行 xing2 這類錯讀;產生器以 OpenCC 詞組級 cn→tw 鏡像後分層,**只輸出 base 音節修正**(2,132 條+12 條 guard),與現行輸出相同者(26,763)冗餘不收、只差聲調者(4,065,一/不變調、兒化、輕聲)留給逐詞審核流程、taiwan profile 已裁決的字位逐位保留 profile 讀音。curation 檔記錄 315,593 句語料 A/B 加 g2pW 意見裁決出的 166 條排除(都會、的當、著眼 這類跨詞邊界為主的字串)與 12 條明列讀音的 guard(不會計較、沒有著急);修改上游 lexicon、review 或 curation 後須重跑 `pnpm generate:matcha-lexicon-traditional`,`test:matcha-lexicon-traditional` 以重新生成比對提交檔作 codegen gate。補充詞典只進 taiwan(產品)frontend,official 保持上游 golden;主詞典 key 優先、review overrides 仍然最後套用。
