...
完整 SQLite run 已掃描 315,593 句與 4,646,998 筆 occurrence；`polyphone` 73,447、`neutral_tone` 118,019、`tone_disagreement` 111,299，三類合計 302,765 筆待審核差異。第五批依完整 SQLite 零反例桶加入 `得` 的 19 個前字與 `為` 的 `以、頗、認、稱` 四個前字 scope，分別新增排除 8,382、2,653 筆，共 11,035 筆，累計約 67,418 筆。profile sync 現會把 contextual allowlist 展開成 phrase decisions；首次同步另讓既有 `著／和` 規則排除 303 筆先前仍出現在 ROI 的 row，但不重複算入本批新校正量。負向測試保護 `獲得 de2`、`因為／為了 wei4`，完整詞 longest-match 仍優先。
