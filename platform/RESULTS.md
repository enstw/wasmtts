...
完整 SQLite run 已掃描 315,593 句與 4,646,998 筆 occurrence；`polyphone` 73,447、`neutral_tone` 118,019、`tone_disagreement` 111,299，三類合計 302,765 筆待審核差異。第一輪完整 ROI 的 11 個固定詞約覆蓋 9,400 筆舊 profile 差異；後兩輪兒化固定詞實際再排除 5,565 筆 `er1` 詞綴語境；兩個多字批次的 15 個校正再排除 28,106 筆，累計約 43,071 筆。第二批 `擁、顫、藉、爺爺、下巴、啞巴、嘴巴、掙錢` 實際排除 6,037 筆，並以負向測試保護 `狼藉 ji2` 與方向義 `東西 xi1`。混合讀音群組持久標為 `needs_context`，不作全域覆寫；`誰 shui2 → shei2` 已確認但 acoustic tokens 缺少 `shei2`，不啟用未支援 phone。
