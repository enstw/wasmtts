...
完整 SQLite run 已掃描 315,593 句與 4,646,998 筆 occurrence；`polyphone` 73,447、`neutral_tone` 118,019、`tone_disagreement` 111,299，三類合計 302,765 筆待審核差異。第一輪完整 ROI 的 11 個固定詞約覆蓋 9,400 筆舊 profile 差異；後兩輪兒化固定詞實際再排除 5,565 筆 `er1` 詞綴語境；四個批次的 33 個校正再排除 41,418 筆，累計約 56,383 筆。第四批 `勁、識、期、質、頗、咋、仔細、彷彿、露出、謝謝、當真` 的原始命中為 14,804 筆；扣除和既有 `期間、期待、認識、意識、見識、資質、材質` 規則重疊後，增量為 10,548 筆。負向測試保護 `露水 lu4` 與 `當前 dang1`，完整詞 longest-match 仍優先於單字 fallback。混合讀音群組持久標為 `needs_context`；`誰 shui2 → shei2` 已確認但 acoustic tokens 缺少 `shei2`，不啟用未支援 phone。
