...
完整 SQLite run 已掃描 315,593 句與 4,646,998 筆 occurrence；`polyphone` 73,447、`neutral_tone` 118,019、`tone_disagreement` 111,299，三類合計 302,765 筆待審核差異。第一輪完整 ROI 採教育部詞條明確、可限制為固定詞的 11 個候選，約覆蓋 9,400 筆舊 profile 差異。`個 ge4` 依臺灣讀音維持目前結果；`兒` 因 `er2` 名詞與 `er1` 詞綴並存，留待按語境拆分；`誰 shui2 → shei2` 已確認但 acoustic tokens 缺少 `shei2`，不啟用未支援 phone。
