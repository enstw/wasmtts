...
完整 SQLite run 已掃描 315,593 句與 4,646,998 筆 occurrence；`polyphone` 73,447、`neutral_tone` 118,019、`tone_disagreement` 111,299，三類合計 302,765 筆待審核差異。第一輪完整 ROI 的 11 個固定詞約覆蓋 9,400 筆舊 profile 差異；後兩輪兒化固定詞實際再排除 5,565 筆 `er1` 詞綴語境；三個批次的 22 個校正再排除 30,870 筆，累計約 45,835 筆。第三批 `倒是、武將、適應、感應、對應、應對、重複` 依序排除 1,722、450、150、128、59、149、106 筆，共 2,764 筆；負向測試保護 `跌倒 dao3`、`將來 jiang1`、`應該 ying1` 與 `重要 zhong4`。`應付` 因教育部辭典同列 `ying1/ying4` 而保留語境審核。混合讀音群組持久標為 `needs_context`，不作全域覆寫；`誰 shui2 → shei2` 已確認但 acoustic tokens 缺少 `shei2`，不啟用未支援 phone。
