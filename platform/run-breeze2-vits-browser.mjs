...
const url = `http://${host}:${serverPort}/platform/breeze2-vits-browser.html`;
const profile = path.join(os.tmpdir(), `wasmtts-breeze2-vits-cdp-${process.pid}`);
const modelRoot = new URL('./models/breeze2-vits/', import.meta.url);
const resultPath = new URL('./results/results-breeze2_vits-browser-wasm.json', import.meta.url);
const wavPath = new URL('./results/breeze2_vits-browser-wasm.wav', import.meta.url);
const artifactNames = ['breeze2-vits.onnx', 'lexicon.txt', 'tokens.txt'];
...
const artifacts = Object.fromEntries(artifactNames.map((name) => {
  const file = new URL(name, modelRoot);
...
try {
  ...
  await waitFor(evalJs, 'window.ready === true', 30000, 'Breeze2 benchmark page');
  ...
  const report = {
    ...
    model: {
      name: 'MediaTek-Research/Breeze2-VITS-onnx',
      revision: metadata.revision,
      architecture: 'VITS distilled from BreezyVoice',
