...
Decision: depthwise-separable FP32 convolution is a viable runtime primitive, unlike the tested INT8 convolution. This does not make the existing Kokoro model faster by graph substitution: new weights must be trained or distilled, and audio quality must be evaluated. The next substantive step is therefore a mobile-vocoder training experiment that replaces the generator's dense residual branches, beginning with the expensive kernel-11 branch. The reproducible probes are generated by `benchmarks/generate-ort-operator-probes.py`; raw profiles are stored under `benchmarks/results/operator-probes/`.
