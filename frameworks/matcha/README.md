...
完整 SQLite run 已掃描 315,593 句與 4,646,998 筆 occurrence，其中 `polyphone`、`neutral_tone`、`tone_disagreement` 合計 302,765 筆待審核差異。第一輪 ROI 的 11 個固定詞約覆蓋 9,400 筆舊 profile 差異；後兩輪兒化固定詞實際再排除 5,565 筆 `er1` 詞綴語境。兩個多字批次加入 `微、血、究、熟、叔、姐姐、差不多、擁、顫、藉、爺爺、下巴、啞巴、嘴巴、掙錢`，再排除 28,106 筆，累計約 43,071 筆。混合讀音群組均持久標為 `needs_context`，不作全域覆寫；`誰 → shei2` 已確認但 acoustic tokens 缺少 `shei2`，維持 `accepted` 而不啟用。
