...
完整 SQLite run 已掃描 315,593 句與 4,646,998 筆 occurrence，其中 `polyphone`、`neutral_tone`、`tone_disagreement` 合計 302,765 筆待審核差異。第一輪 ROI 的 11 個固定詞約覆蓋 9,400 筆舊 profile 差異；第二輪再以 `會兒、今兒、這兒、個兒、那兒` 覆蓋 3,906 筆 `er1` 詞綴語境，兩輪合計約 13,306 筆。`個 ge4` 依臺灣讀音維持目前結果；`兒童、嬰兒、女兒` 等名詞仍維持 `er2`；`誰 → shei2` 已確認但 acoustic tokens 缺少 `shei2`，維持 `accepted` 而不啟用。
