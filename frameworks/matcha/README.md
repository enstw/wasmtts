...
完整 SQLite run 已掃描 315,593 句與 4,646,998 筆 occurrence，其中 `polyphone`、`neutral_tone`、`tone_disagreement` 合計 302,765 筆待審核差異。第一輪 ROI 的 11 個固定詞約覆蓋 9,400 筆舊 profile 差異；後兩輪兒化固定詞實際再排除 5,565 筆 `er1` 詞綴語境。三個批次加入 `微、血、究、熟、叔、姐姐、差不多、擁、顫、藉、爺爺、下巴、啞巴、嘴巴、掙錢、倒是、武將、適應、感應、對應、應對、重複`，再排除 30,870 筆，累計約 45,835 筆。第三批七詞依序排除 1,722、450、150、128、59、149、106 筆，共 2,764 筆。混合讀音群組均持久標為 `needs_context`，不作全域覆寫；`應付` 因教育部同列 `ying1/ying4` 保留語境審核，`誰 → shei2` 已確認但 acoustic tokens 缺少 `shei2`，維持 `accepted` 而不啟用。
