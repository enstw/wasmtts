...
Decision: depthwise-separable FP32 convolution is a viable runtime primitive, unlike the tested INT8 convolution. This does not make the existing Kokoro model faster by graph substitution: new weights must be trained or distilled, and audio quality must be evaluated. The next substantive step is therefore a mobile-vocoder training experiment that replaces the generator's dense residual branches, beginning with the expensive kernel-11 branch. The reproducible probes are generated by `platform/generate-ort-operator-probes.py`; raw profiles are stored under `platform/results/operator-probes/`.
...
For local reproduction, start `pnpm host:mobile`, then run:
...
The 323 MB model is intentionally not part of the handoff commit. Restore it at `platform/models/kokoro-fp32-download/onnx/model.onnx` and verify SHA-256 `94b973941b1852754f979be5d5e20be666d5c81d9bb886b88ae1dc85c9b895ca`. If real iOS Safari cannot expose two workers reliably, return to the now runtime-validated mobile-vocoder direction. The next step there is training or distilling a generator with depthwise-separable residual blocks and comparing both quality and end-to-end WASM latency; further synthetic kernel probing is no longer the bottleneck.
